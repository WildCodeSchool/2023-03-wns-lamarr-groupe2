...
const Tags: FC<TagsProps> = ({ isDisabledContenders, setSelectedTags, selectedTags }) => {
    ...
    return (
        <div className="mt-12 w-full">
            ...
            <Select
                ...
                menuPlacement="top"
            />
            <button className=' hidden md:block font-content  underline text-small-p  pr-16' onClick={() => setIsShowAll(prev => !prev)}>{isShowAll ? 'réduire' : `voir tous (${selectedTags.length})`}</button>

            <div className="hidden : md:flex flex-wrap gap-3 mt-10">
                {selectedTags.slice(0, isShowAll ? selectedTags.length : 0).map((tag) => (
                    <div key={tag.id} className={`bg-${tag.color}  px-2 customBorder rounded-none gap-4 flex justify-center items-center `}>
                        <p>{tag.label}</p>
                        <button onClick={() => handleSelectedContenders(tag.id)} className="font-content ">X</button>
                    </div>
                ))}
            </div>
        </div>
