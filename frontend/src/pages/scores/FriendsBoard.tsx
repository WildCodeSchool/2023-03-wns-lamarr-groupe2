        ...
        <table className="table-fixed w-full">
          <thead>
            <tr className=" h-32">
              ...
              <th className="w-2/12 xl:w-3/12 font-thin text-[24px] hidden  md:table-cell">
                CHALLENGES RÉALISÉS
              </th>
              <th className="w-2/12 xl:w-2/12 font-thin text-[24px] md:text-left">
                POINTS
              </th>
          ...
          <tbody className="">
            {isEmpty(filteredFriendList) ? (
              <EmptyRow />
            ) : (
              filteredFriendList
                .slice(indexOfFirsFriend, indexOfLastFriend)
                .map((friend, index) => (
                  <BoardRow
                    key={index}
                    index={index + indexOfFirsFriend}
                    friend={friend}
                  />
                ))
            )}
        ...
        {pageNumbers.length > 1 && (
          <div className="flex gap-12">
            <button
              ...
              className={
                currentPage === 1 ? " text-tertiary-dark" : "text-primary-good"
              }
            >
              ⬅ PRÉCÉDENT
            </button>
