...
const UsersList = () => {
    ...
    const handleToggleSelection = (user: UserGlobal) => {
        setSelectedUserIds((prevId) =>
            prevId === user.id ? undefined : user?.id
        );
    }
    ...
    const filteredUsers = useMemo(() => {
        return pipe(
            users,
            filter((user) => tokens.every((token) =>
    ...
    return (
        <>
            <div className="mb-3">
                <InputCustom mode='search' type="text" name='' value={searchUser} onChange={handleSearchUser} />
            </div>
            <div className="py-10 h-60 overflow-y-auto">
                {filteredUsers?.map((user, index) => (
                    <div key={index} className="border-b flex justify-between" onClick={() => handleToggleSelection(user)}>
            ...
            <div className="flex justify-center h-20 items-center mb-6">
                {selectedUserIds && <BtnCustom text="AJOUTER" styled="btnGood" onClick={() => handleAddFriend(selectedUserIds)} />
                }</div> </>
    )
}
