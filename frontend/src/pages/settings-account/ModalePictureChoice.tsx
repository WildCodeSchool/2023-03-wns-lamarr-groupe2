...
export const ModalePictureChoice : FC<{setOpenPictureChoice :Dispatch<SetStateAction<boolean>>}>= ({setOpenPictureChoice}) => {
    const {user, updatePicture} = useUserContext()
    const [selectedOption, setSelectedOption] = useState<string | 'hermesG'>(user?.picture || 'hermesG');
    
    const handleSelect = (option: string) => {
     console.log('Selected Option : ', selectedOption)
      setSelectedOption((prevOption) => (prevOption === option ? 'hermesG' : option));
        ...
        <div className="relative transform  w-full overflow-hidden rounded-lg border-[1px] border-main-white bg-main-dark text-main-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
          <div className="bg-main-bg h-full  px-0 w-full pt-2 pb-4 sm:p-6 sm:pb-4 ">
                                  ...
                                  <div className='flex  justify-center'>
                                    <div onClick={() => setOpenPictureChoice(prev => !prev)} className="absolute  left-5 bg-light  h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-main-white sm:mx-0 sm:h-10 sm:w-10">
            ...
            <div className="mt-2 w-full">
                 <ul className='flex w-full justify-center'>
                    {['hermesG', 'hermesR', 'hermesY'].map((choice, index) => <PictureChoice
            key={index}
            hermesChoice={choice}
            last={index}
