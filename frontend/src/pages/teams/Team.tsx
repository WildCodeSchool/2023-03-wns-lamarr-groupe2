...
const Team: FC<{ team: TTeam }> = ({ team }) => {
    ...
    return (
        <>
            <div className={`h-16 w-16 md:h-20 md:w-20 lg:h-28 lg:w-28 flex justify-center items-center uppercase rounded-full border-1 ${team?.color}`} onClick={() => setIsOpenModale(prev => !prev)}>{acronymGenerator(team?.company_name)}</div>
